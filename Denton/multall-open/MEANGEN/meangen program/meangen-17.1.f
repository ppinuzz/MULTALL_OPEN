...
C
      ...
      RGAS       = 287.15  ! GAS CONSTANT, VALUE FOR AIR.
      GAMM       = 1.4     ! GAS SPECIFIC HEAT RATIO, VALUE FOR AIR.
      ...
      NSMOOTH    = 4       ! NUMBER OF SMOOTHINGS OF THE STREAM SURFACE COORDINATES.
...
C
      ...
      WRITE(6,*)'******************************************************'
      WRITE(6,*)
      WRITE(6,*) '  ARE THE ANGLES, MASS FLOW, DESIGN RADIUS,EFFICIENCY'
      WRITE(6,*) ', ETC, FOR THIS STAGE  "ALL" THE SAME AS FOR THE LAST'
      WRITE(6,*) '  STAGE ? '
      WRITE(6,*) '  ANSWER  Y  or  N . '
           READ(5,*)      IFSAME_ALL
           WRITE(6,*)   ' IFSAME_ALL= ', IFSAME_ALL
           WRITE(10,109)  IFSAME_ALL
  109 FORMAT(A1,T25,' IFSAME_ALL, SET = "Y" TO REPEAT THE LAST STAGE VEL
     &OCITY TRIANGLES ')
...
C
      ...
      ELSE 
           XMEAN(1) = XMEAN(7) +  0.333*STAGEGAP(NSTG)*AXCHRD1(NSTG)
           XMEAN(2) = XMEAN(7) +  0.667*STAGEGAP(NSTG)*AXCHRD1(NSTG)
           XMEAN(3) = XMEAN(7) +        STAGEGAP(NSTG)*AXCHRD1(NSTG)
...
C
      DO 155 NS = NTE2,NSS
              VTLOC     = VT_TE2*RMEAN(NTE2)/RMEAN(NS)
              WTLOC     = VTLOC - U_LOC(NS)
              VMLOC     = VM_LOC(NS)
              VABS(NS)  = SQRT(VTLOC*VTLOC + VMLOC*VMLOC)
...
C
      DO 154 NS = NLE1+1,NTE1
              ...
              VTLOC     = VT_LE1 + FRAC*(VT_TE1 - VT_LE1)
              WTLOC     = VTLOC - U_LOC(NS)
              VMLOC     = VM_LOC(NS)
              VABS(NS)  = SQRT(VTLOC*VTLOC + VMLOC*VMLOC)
              VREL(NS)  = SQRT(WTLOC*WTLOC + VMLOC*VMLOC)
              HOLOC(NS) = HO(NSTG) + U_LOC(NS)*(VTLOC - VT_LE1)
...
C
      DO 157 NS = NTE1+1,NLE2
              VTLOC     = VT_TE1*RMEAN(NTE1)/RMEAN(NS)
              WTLOC     = VTLOC - U_LOC(NS)
              VMLOC     = VM_LOC(NS)
              VABS(NS)  = SQRT(VTLOC*VTLOC + VMLOC*VMLOC)
              VREL(NS)  = SQRT(WTLOC*WTLOC + VMLOC*VMLOC)
...
C
      DO 156 NS = NLE2,NTE2
              ...
              VTLOC     = VT_LE2 + FRAC*(VT_TE2 - VT_LE2)
              WTLOC     = VTLOC - U_LOC(NS)
              VMLOC     = VM_LOC(NS)
              VABS(NS)  = SQRT(VTLOC*VTLOC + VMLOC*VMLOC)
              VREL(NS)  = SQRT(WTLOC*WTLOC + VMLOC*VMLOC)
              HOLOC(NS) = HOMID(NSTG) + U_LOC(NS)*(VTLOC - VT_LE2)
...
C   SKIP ANY OVERLAPPING POINTS
      DISTMIN = 0.05*(AXCHRD1(NSTG) + AXCHRD2(NSTG))
...
C
      CALL SMOOTH(1,NINTPTS,NSMOOTH,SFAC,SDISTHUB,XSURFHUB)
      CALL SMOOTH(1,NINTPTS,NSMOOTH,SFAC,SDISTHUB,RSURFHUB)
      CALL SMOOTH(1,NINTPTS,NSMOOTH,SFAC,SDISTTIP,XSURFTIP)
      CALL SMOOTH(1,NINTPTS,NSMOOTH,SFAC,SDISTTIP,RSURFTIP)
C
...
C
 ...
 9003 FORMAT(2F12.4,T25, '  RFRAT,  FRMAX')
C
      WRITE(9,9005) 0  
...
C     SET BLADE ANGLES FOR THE FIRST ROW
      IF(MOD(NR,2).GT.0)  THEN
      IF(BIN_ROW1(NSTG).GT.BOUT_ROW1(NSTG)) THEN
           BLE   = BIN_ROW1(NSTG)  - AINC_1*DEGRAD
           BTE   = BOUT_ROW1(NSTG) - DEVN_1*DEGRAD
      ELSE
           BLE   = BIN_ROW1(NSTG)  + AINC_1*DEGRAD
           BTE   = BOUT_ROW1(NSTG) + DEVN_1*DEGRAD
      ENDIF
      END IF
C
C SET BLADE ANGLES FOR THE SECOND ROW 
      IF(MOD(NR,2).EQ.0) THEN            
      IF(BOUT_ROW2(NSTG).GT.BIN_ROW2(NSTG)) THEN
           BLE  = BIN_ROW2(NSTG)   + AINC_2*DEGRAD
           BTE  = BOUT_ROW2(NSTG)  + DEVN_2*DEGRAD
      ELSE
           BLE  = BIN_ROW2(NSTG)   - AINC_2*DEGRAD
           BTE  = BOUT_ROW2(NSTG)  - DEVN_2*DEGRAD
      ENDIF
...
C  
      IF(ROWTYP.EQ.'S') THEN
                BLE   = ATAN( TAN(BLE)*RDESIGNLE/RSECTLE )
                BTE   = ATAN( TAN(BTE)*RDESIGNTE/RSECTTE )
      END IF
C
      IF(ROWTYP.EQ.'R') THEN
               RRAT     = RDESIGNLE/RSECTLE
               PHILEE   = PHI_LOC(NLE)
               TAN_BABS = TAN(BLE) + 1.0/PHILEE
...
C    THE DISTANCE IS TRANSFORMED BY  "EXPO". 
C   
      TAN1 = TAN(BLE)
      TAN6 = TAN(BTE)
