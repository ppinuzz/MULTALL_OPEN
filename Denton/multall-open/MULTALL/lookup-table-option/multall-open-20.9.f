...
C
     ...
     &            BALPHA1,BALPHA2)
      ...
      END IF
C
C    ESTIMATE THE INTERNAL ENERGY  UINT(J,K)   FROM AN ISENTROPIC PROCESS.
C
      IF(IFGAS.EQ.3) THEN
          ROS  = ROSTAG_IN(K)*PRAT**(1.0/GA_PV_AVG)            
          IF(J.EQ.1) THEN
                     DU = (PGUESS(1) - PO1(K))/ROS/GA_PV_IN
          ELSE
                     DU = (PGUESS(J) - PGUESS(J-1))/ROS/GA_PV_NOW
          END IF
          UNOW = UNOW + DU
          CALL TABSEARCH(ROS,UNOW,
     &                 PNOW,TNOW,ENTPY_NOW,GA_PV_NOW,DRYNOW)
          TS        = TNOW
