...
C   OF THE  PARAMETER STATEMENTS IN "commall-open-20.6"                C
C                                                                      C
...
C
      DO 5900 K=1,KM
C    CHANGE LIMIT TO JM FROM JMM1 TO JM
      DO 5900 J=2,JM
      DO 5900 I=1,IM
           RONOW = RO(I,J,K)
...
C
      ...
      IF(IFGAS.EQ.0) THEN
        ROE(I,JM,K) = ROE(I,JM,K) + EINT*DROWAVE + ROOLD*(CV*DTWAVE 
     &              + VEXIT*DVWAVE)
        HO(I,JM,K)  = GA*(ROE(I,JM,K)/RONEW - EKE)  + EKE
C        HO(I,JM,K) = HO(I,JM,K) + CP*DTWAVE + VEXIT*DVWAVE
      ELSE
...
C
      IF(ABS((A2_SPEC*RADDEG)).GT.30.0) THEN
C
      DO NL = 1,5  
      COSA2_NEW  =  DFORCE/
     & (UBLAD2/WREL_OUT + SINA2_COMP - RAVG_IN*VT_IN/RAVG_OUT/WREL_OUT)
      IF(COSA2_NEW.GT.0.999)  COSA2_NEW = 0.999
      IF(COSA2_NEW.LT.0.0)    COSA2_NEW = 0.0
      SINA2_NEW  = SQRT(1.0- COSA2_NEW*COSA2_NEW)*SGNOUT 
      SINA2_COMP = 0.9*SINA2_COMP + 0.1*SINA2_NEW
      END DO
      COSA2_COMP = COSA2_NEW
C
      ELSE
C
      DO NL = 1,5
      SINA2_NEW = DFORCE/COSA2_COMP 
     &          - UBLAD2/WREL_OUT + RAVG_IN*VT_IN/RAVG_OUT/WREL_OUT
      IF(SINA2_NEW.GT.0.999)  SINA2_NEW = 0.999
      IF(SINA2_NEW.LT.-0.999) SINA2_NEW = -0.999
      COSA2_NEW   = SQRT(1.0 - SINA2_NEW*SINA2_NEW)
      COSA2_COMP  = 0.9*COSA2_COMP + 0.1*COSA2_NEW
      END DO
      SINA2_COMP = SINA2_NEW
C
      END IF
C
C        END OF ITERATION.
