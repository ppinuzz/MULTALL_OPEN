...
C   OF THE  PARAMETER STATEMENTS IN "commall-open-17.1"                C
C                                                                      C
...
C
      IF(IF_RESTART.NE.0.AND.NSTEP.GT.1) GO TO 5555
C
      FCHANGE     = 1.0 -  FLOAT(NSTEP-1)/NCHANGE
C
      IF(FCHANGE.GT.0.0)   THEN
           IF(IF_RESTART.NE.0) FCHANGE = 0.0
...
C
      ...
      IF(FDEVN.GT.1.0) FDEVN = 1.0 
      IF(FDEVN.LT.0.0) FDEVN = 0.0
      FLOAD = 1.0
      IF(J.LT.JLEE-1) FLOAD = 0.0
      IF(J.GT.JTEE+1) FLOAD = 0.0
C  
      DO 5553 K=1,KMM1
      GRID_ANGL  = ALPHA_CENT(J,K)
      DEV_ANGL   = DEVN_ANGL(NR,K)*FDEVN
      ALPHA_FLOW = ALPHA_CENT(J,K) - DEV_ANGL
      ROVMAVG    = FLOWX(1,J,K)/AQTOT(1,J,K) 
...
C     TO OUTPUT FILE IF USING ARTIFICIAL COMPRESSIBILITY.
      IF(ITIMST.EQ.5.OR.ITIMST.EQ.6) THEN
...
C      WRITE OUT THE RATIO OF TURBULENT TO LAMINAR VISCOSITY
C      WRITE(7)(((VISC_RAT(I,J,K),I=1,IM),J=1,JM),K=1,KM)
...
C     SET THE CENTRE LINE ANGLES UPSTREAM AND DOWNSTREAM OF A BLADE ROW.
      ...
      JLEE  = JLE(NRW)
      JTEE  = JTE(NRW) 
      AVGLE = (CENT_ANGL(JLEE)+CENT_ANGL(JLEE+1)+CENT_ANGL(JLEE+2))/3
      AVGTE = (CENT_ANGL(JTEE)+CENT_ANGL(JTEE-1)+CENT_ANGL(JTEE-2))/3
      IF(J.LE.JLEE) CENT_ANGL(J) = AVGLE
      IF(J.GE.JTEE) CENT_ANGL(J) = AVGTE
 9036 CONTINUE
C
...
C
      IF(I.EQ.IMID.AND.J.EQ.JTEDGE) THEN
           ...
           CHORD(NRW)    = SQRT(SCHORD*SCHORD + TCHORD*TCHORD)
...
C     ALLOW FOR ROUGHNESS IF ROUGH > 1.0E-7 .
      IF(ROUGH.GT.1.0E-7) THEN
           TW1     = 0.5*CF*ROAVG(2)*WBOUND(2)*WBOUND(2)
           VSTAR   = SQRT(TW1/ROAVG(2))
...
C
           TW1 = CFWALL*ROAVG(1)*WBOUND(1)*WBOUND(1)
C
      ENDIF
C
C************************************************************************
...
C     ALLOW FOR ROUGHNESS IF ROUGH > 1.0E-7 .
      IF(ROUGH.GT.1.0E-7) THEN
           TWM    = 0.5*CF*ROAVG(KMM1)*WBOUND(KMM1)*WBOUND(KMM1)
           VSTAR   = SQRT(TWM/ROAVG(KMM1))
...
C   IF YPLUSWALL > 5 USE THE YPLUSWALL VALUE TO CALCULATE THE SKIN FRICTION
C.
           TWM = CFWALL*ROAVG(KM)*WBOUND(KM)*WBOUND(KM)
C
      ENDIF
C
C******************************************************************************
...
C     ALLOW FOR ROUGHNESS IF ROUGH > 1.0E-7
      IF(ROUGH.GT.1.0E-7) THEN
           TWI1    = 0.5*CF*ROAVG(2)*WBOUND(2)*WBOUND(2)
           VSTAR   = SQRT(TWI1/ROAVG(2))
           PLUSK   = ROUGH*VSTAR*ROAVG(2)/VISLAM
           RELOG10 = LOG10(PERP/ROUGH)
           FUNCN   = 5.75*RELOG10 + 8.5
           CF_FULL_ROUGH = 2.0/(FUNCN*FUNCN)
...
C
      ELSE
C   IF YPLUSWALL > 5 USE THE YPLUSWALL VALUE TO CALCULATE THE SKIN FRICTION
      TWI1    = CFWALL*ROAVG(1)*WABS(1)*WABS(1)
C
      ENDIF
C******************************************************************************
C******************************************************************************
...
C    ALLOW FOR ROUGHNESS IF  ROUGH > 1.0E-7 .
      IF(ROUGH.GT.1.0E-7) THEN
           TWIM    = 0.5*CF*ROAVG(IMM1)*WBOUND(IMM1)*WBOUND(IMM1)
           VSTAR   = SQRT(TWIM/ROAVG(IMM1))
...
C
      ELSE
C   IF YPLUSWALL > 5 USE THE YPLUSWALL VALUE TO CALCULATE THE SKIN FRICTION
           TWIM   = CFWALL*ROAVG(IM)*WABS(IM)*WABS(IM)
C
      ENDIF
C
C******************************************************************************
...
C   END OF JAN/14 ADDITION
C
      FMULT     = TWI1*AREA(1)/WABS(1)
      XSTRES(1) = FMULT*VXAVG(1)
      RSTRES(1) = FMULT*VRAVG(1)
      TSTRES(1) = FMULT*WTAVG(1)
C
      FMULT     = -TWIM*AREA(IM)/WABS(IM)
...
C
      DO 5640 J=1,JM
      RATIO    = SUMAS(J)/(SUMAS(1) + SUMCWL(J))
...
C
      ...
      TDIF  = RTHETA(I,J,K) - RTHETA(1,JSURF,KSURF)
      DISTSQ = XDIF*XDIF + RDIF*RDIF + TDIF*TDIF
      IF(DISTSQ.LT.DMIN) THEN
           DMIN = DISTSQ
           JMIN = JSURF
           KMIN = KSURF
...
C     FIND THE PERPENDICULAR DISTANCE TO THE NEAREST POINT ON THE I=1 BLADE SURFACE.
     ...
     &      + ABT(JMIN,KNORM)*ABT(JMIN,KNORM))
      ...
      TDIF   = RTHETA(I,J,K) - RTHETA(1,JMIN,KMIN)
...
C
      ...
      XDIF  = X(J,K)  - X(JSURF,KSURF)
      RDIF  = R(J,K)  - R(JSURF,KSURF)
      TDIF  = RTHETA(I,J,K) - RTHETA(IM,JSURF,KSURF)
      DISTSQ = XDIF*XDIF + RDIF*RDIF + TDIF*TDIF
      IF(DISTSQ.LT.DMIN) THEN
           DMIN = DISTSQ
           JMIN = JSURF
...
C     FIND THE PERPENDICULAR DISTANCE TO THE NEAREST POINT ON THE I=IM BLADE SURFACE.
     ...
     &     + ABT(JMIN,KNORM)*ABT(JMIN,KNORM))
      ...
      XDIF  = X(J,K)  - X(JMIN,KMIN)
      RDIF  = R(J,K)  - R(JMIN,KMIN)
      TDIF  = RTHETA(I,J,K) - RTHETA(IM,JMIN,KMIN)
...
C
     ...
     &      + VT(I,JM1,K)+VT(IP1,JM1,K)+VT(IP1,JM1,KP1)+VT(I,JM1,KP1))
...
C
      ...
      CALL GRADVEL(I,J,K,VT,DVTDX,DVTDR,DVTDT)
...
C     
      ...
      TXX(I,J,K) = DVXDX
      TXT(I,J,K) = DVTDX + DVXDT
      TRR(I,J,K) = DVRDR
      TRT(I,J,K) = DVTDR + DVRDT - VTAVG/RAVG
      TTT(I,J,K) = DVTDT
      QXX(I,J,K) = DTEMPDX
      QRR(I,J,K) = DTEMPDR
      QTT(I,J,K) = DTEMPDT
...
C
      IF(ROUGH.GT.1.0E-7) THEN
           TWI1    = CF*ROAVG1*WAVG*WAVG/128
           VSTAR   = SQRT(TWI1/ROAVG1/4)
...
C
      IF(ROUGH.GT.1.0E-7) THEN
           TWIM    = CF*ROAVGIM*WAVG*WAVG/128
           VSTAR   = SQRT(TWIM/ROAVGIM/4)
...
C
      ...
      WTAVG1 = WT(I,J,2)+WT(IP1,J,2)+WT(IP1,JM1,2)+WT(I,JM1,2)
...
C
      IF(ROUGH.GT.1.0E-7) THEN
           TW1     = CF*ROAVG1*WAVG*WAVG/128
           VSTAR   = SQRT(TW1/ROAVG1/4)
           PLUSK   = ROUGH*VSTAR*ROAVG1/VISLAM/4
           RELOG10 = LOG10(DPERP/ROUGH)
           FUNCN   = 5.75*RELOG10 + 8.5
           CF_FULL_ROUGH = 2.0/(FUNCN*FUNCN)
...
C
     ...
     &        + VR(IP1,JM1,KMM1)+VR(I,JM1,KMM1)
      WTAVGKM = WT(I,J,KMM1)+WT(IP1,J,KMM1)
     &        + WT(IP1,JM1,KMM1)+WT(I,JM1,KMM1)
...
C
      IF(ROUGH.GT.1.0E-7) THEN
           TWKM    = CF*ROAVGKM*WAVG*WAVG/128
           VSTAR   = SQRT(TWKM/ROAVGKM/4)
...
C
      VSTAR   = SQRT(TWALLI1/(0.25*ROAVG1))
      DO I = 1,IMID
      YPLS = RO(I,J,K)*VSTAR*SQRT(DWALLSQ(I,J,K))/VISLAM
      Y_PLUS(I,J,K) = AMIN1(1000.0,YPLS)
      END DO
C        
      FMULT   = -TWALLI1*AREA1/WAVG
