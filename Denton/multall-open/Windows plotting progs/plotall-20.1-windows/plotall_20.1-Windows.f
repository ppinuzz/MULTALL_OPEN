...
C                                                                  C
C     (95,2500,95) to (IMAX,JMAX,KMAX)                              C
C     (95,2500)    to (IMAX,JMAX)                                   C
C        (2500,95) to      (JMAX,KMAX)                              C
C     (95)        to (IMAX)                                        C
C        (2500)    to      (JMAX)                                   C
C            (95) to           (KMAX)                              C
C        (95,2500) to      (KMAX,JMAX)                              C
C                                                                  C
...
C
      ...
      COMMON / BLK2 / X(2500,95),T(95,2500,95),R(2500,95),
     &                SM(2500,95),PITCH(2500,95),COSA(2500),SINA(2500)
      COMMON / BLK6 / VX(95,2500,95),VT(95,2500,95),VR(95,2500,95)
      COMMON / BLK9 / P(95,2500,95)
      COMMON / BLK10/ RO(95,2500,95)
      COMMON / BLK11/ MACH(95,2500,95)
      COMMON / BLK14/ Q(95,2500,95),QQ(95,2500,95),NQ,NQQ
      COMMON / BLK15/ IM,JM,KM,ISPLIT,KSPLIT,JLE,JTE,JSLE,JSTE,CP,GA,
     &                W(2500),IND(2500),NBLADES(2500),PI,JLEE(50),
     &                JTEE(50),NROWS
      COMMON / BLK16/ ALPHA1,ALPHA2,EMACH,IMACH
      COMMON / BLK19/ XX(95,2500),YY(95,2500),ZZ(95,2500),
     &                WORK1(95,2500),WORK2(95,2500)
      COMMON / BLK20/ XS1(2500),YS1(2500),XS2(2500),YS2(2500)
      COMMON / BLK21/ XMIN,XMAX,RTMIN,RTMAX,YMIN,YMAX,SMMIN,SMMAX,
     &                ZMIN,ZMAX,RTMID,TMID,SRMIN,SRMAX,RMIN,RMAX,ZLAST
      COMMON /BLKPRIM/ ROVX(95,2500,95),ROVR(95,2500,95),
     &                 ROVT(95,2500,95),ROE(95,2500,95),
     &                 DUM(95,2500,95)
C
      DIMENSION PROP(95,2500,95)
C
      CHARACTER*32 NPROP,NQ,NQQ
      CHARACTER*8  FMT
      CHARACTER*72 JBNAME,TSNUM
C
      REAL MACH,EMACH,IMACH
C
      CHARACTER*72 INFILE
C
...
C
      ...
      COMMON / OPT1 / I1, J1, K1, DELR1, ZSCAL1, I1Z, J1Z, K1Z, ANS1, 
     &                CTYPE1, IPIT1, VMOD, NDIFF
C
...
C      INFILE = 'flow_out'
C      WRITE (6,2080)
C      READ  (5,1080,ERR=5,END=5) INFILE
C    5 CONTINUE
...
C
C    JUMP TO HERE FOR A NEW OUTPUT
C
   99 CONTINUE
C
C
...
C 
  ...
  250 CONTINUE
C
C    DO NOT CLOSE 21 FOR MULTIPLE GROD INPUTS
C      CLOSE(21)
C
      NLETE = 0
      DO 200 J=1,JM
...
C
     ...
     &                JTEE(50),NROWS
C
      DIMENSION XOCX1(2500),XOCX2(2500)
C
      CHARACTER*32 NXOCX
C
...
C
     ...
     &                JTEE(50),NROWS
C
      DIMENSION SOS01(2500),SOS02(2500)
C
      CHARACTER*32 NSOS0
C
...
C*****************************************************************************
      ...
      VT(I,J,K) = ROVT(I,J,K)/RO(I,J,K) - W(J)*R(J,K)
      VR(I,J,K) = ROVR(I,J,K)/RO(I,J,K)
      EINT      =  ROE(I,J,K)/RO(I,J,K)
      VTABS     =  ROVT(I,J,K)/RO(I,J,K)
      WTREL     =  VT(I,J,K)
      VMSQ      =  VX(I,J,K)*VX(I,J,K) + VR(I,J,K)*VR(I,J,K)
      EKE       =  VMSQ + VTABS*VTABS 
      EKEREL    =  VMSQ + WTREL*WTREL 
      TSTAT     =  (EINT - 0.5*EKE)/CV
      VSSQ      =  GA*RGAS*TSTAT
      MACH(I,J,K) = SQRT(EKEREL/VSSQ)
      P(I,J,K)    = RGAS*TSTAT*RO(I,J,K)
      IF(I.EQ.1.AND.K.EQ.5) WRITE(6,*) 'ROE, TSTAT, RO, PRESS',
     &          ROE(I,J,K),TSTAT,RO(I,J,K),P(I,J,K)
  100 CONTINUE
C
      RETURN
C
...
C
      DIMENSION PROP(95,2500,95),HITES(125),STOREX(2500,95),
     &          STORER(2500,95),ZZZ(95,2500),HITES2(10)
C
C
      REAL EMACH,IMACH
C
...
C
      ...
      COMMON / OPT1 / I, J, K, DELR, ZSCALE, IZ, JZ, KZ, ANS, 
     &                CTYPE, IPIT, VMOD, NDIFF
C
C     DATA STATEMENT
C
      NROW = (95)
...
2000  FORMAT(/' TYPE :'/
     ...
     + '  "2"  -  TANGENTIAL VELOCITY      "18"  -  R * VTHETA-ABS    '/
     + '  "3"  -  DENSITY                  "19"  -  EXP (-S/R )       '/ 
     + '  "4"  -  STATIC PRESSURE          "20"  -  ABSOLUTE To       '/
     ...
     + '  "9"  -  TRANSFORMED VISCOSITY     OR      VISCOSITY RATIO   '/                  
     + ' "10"  -  VELOCITY                 "25"  -  ABSOLUTE VELOCITY '/
     + ' "11"  -  STATIC TEMPERATURE       "26"  -  ABSOLUTE MACH No. '/ 
     + ' "12"  -  RELATIVE STAGn. TEMP.    "27"  -  ABS. PITCHWISE ANG'/ 
     ...
     + ' "33"  -  SPARE OUTPUT VARIABLE.   "34"  - ST TUBE THICKNESS '/)
C
2001  FORMAT(   
...
C
     ...
     &                JTEE(50),NROWS
      ...
      COMMON / BLK20/ XS1(2500),YS1(2500),XS2(2500),YS2(2500)
C
      DIMENSION PROP(95,2500,95),XSS1(2500),XSS2(2500)
C
...
C
     ...
     &                JTEE(50),NROWS
      COMMON / BLK16/ ALPHA1,ALPHA2,EMACH,IMACH
C
      DIMENSION PROP(95,2500,95),PROP2(95,2500,95),ROVM(2500),
     &          POINPUT(2500), TOINPUT(2500), PROP3(95,2500,95),
     &  FRACFLO(2500,95),FFLO_REF(95),TKSS_REF(95),FFLO(95),TKSS(95)
C
...
C
C
C       Calculate other flow variables
C
C
C
C
...
C
    ...
    4   CONTINUE
        FMT         = '(F9.1)  '
        NPROP       = '       STATIC PRESSURE '
C
C        '
      ELSE IF(IREPLY . EQ . 5) THEN
C
        WRITE(6,*) ' STAGN PRESSURE OR LOSS COEFFICIENT'
        WRITE(6,*) ' ANSWER P  or L '
        READ(5,*)    ANS_LOS
C        
        DO 5 K=1,KM
        DO 5 J=1,JM
        DO 5 I=1,IM
          FMACH  = (1.0+(GA-1.0)*0.5*MACH(I,J,K)**2)
          PROP2(I,J,K)  = P(I,J,K)*FMACH**(GA/(GA-1.0)) 
          PROP3(I,J,K)  = P(I,J,K) 
          PROP(I,J,K)   = PROP2(I,J,K)
    5   CONTINUE
C
        IF(ANS_LOS.EQ.'L'.OR.ANS_LOS.EQ.'l') THEN
             WRITE(6,*) ' INPUT THE REFERENCE J VALUE FOR THE STAGNATION
     & PRESSURE '
             READ(5,*)    JLOS1
             CALL SUB23(PROP2,0)
             CALL SUB23(PROP3,0)
             ...
             DO 55 K=1,KM
             DO 55 I=1,IM
             ROAVG    = 0.5*(RO(I,J,K)+RO(I,JLOS1,K))
             VBLADE1  = W(J)*R(JLOS1,K)
             VBLADE2  = W(J)*R(J,K)
             DPOREL   = 0.5*ROAVG*(VBLADE2*VBLADE2 - VBLADE1*VBLADE1)
             PROP(I,J,K) = (PROP2(I,JLOS1,K) - PROP2(I,J,K) + DPOREL)/
     &                     (PROP2(I,J,K) - PROP3(I,J,K))
   55        CONTINUE
C
        ELSE
            ANS_LOS = 'P'
...
C
        ...
        IF(ANS_LOS.EQ.'P') THEN
             NPROP = ' RELATIVE STAGNATION PRESSURE   '
             FMT         = '(F10.1)  '
        ELSE
             NPROP = ' STAGN. PRESSURE LOSS COEFFICIENT  '
             FMT         = '(F10.5)  '
        END IF
C
C
...
C
      ELSE IF(IREPLY . EQ . 9) THEN
        ...
        DO 9 J=1,JM
        DO 9 I=1,IM
C          PROP(I,J,K) = VT(I,J,K)+W(J)*R(J,K)
           PROP(I,J,K) = Q(I,J,K)
    9   CONTINUE
        FMT         = '(F8.5)'
        NPROP       = ' VISCOSITY RATIO  '
C
C
   ...
   10   CONTINUE
      ...
      ELSE IF(IREPLY . EQ . 11) THEN
        ...
        DO 11 J=1,JM
        DO 11 I=1,IM
          PROP(I,J,K) = P(I,J,K)/(RO(I,J,K)*GC)
   11   CONTINUE
        FMT         = '(F7.1)  '
        NPROP       = '       STATIC TEMPERATURE       '
      ELSE IF(IREPLY . EQ . 12) THEN
        DO 12 K=1,KM
        DO 12 J=1,JM
        DO 12 I=1,IM
          FMACH  = (1.0+(GA-1.0)*0.5*MACH(I,J,K)**2)
          PROP(I,J,K) = P(I,J,K)/(RO(I,J,K)*GC) * FMACH
   12   CONTINUE
        FMT         = '(F7.1)  '
        NPROP       = 'RELATIVE STAGNATION TEMPERATURE '
C
C
...
C
        ...
        DO 19 J=1,JM
        DO 19 I=1,IM
          PROP(I,J,K) = ( ( P(IMID,1,KMID)  /  P(I,J,K) )*
     &                    ( RO(I,J,K) / RO(IMID,1,KMID) )**GA )
     &                                            **(1.0/(GA-1.0))
   19   CONTINUE
        FMT         = '(F8.4)  '
        NPROP       = '      EXP ( -S / R )            '
      ELSE IF(IREPLY . EQ . 20) THEN
        ...
        DO 20 I=1,IM
          FMACH       = (1.0+(GA-1.0)*0.5*MACH(I,J,K)**2)
          T0REL       = P(I,J,K)/(RO(I,J,K)*GC) * FMACH
          U           = W(J)*R(J,K)
          PROP(I,J,K) = T0REL + ( U*VT(I,J,K) + 0.5*U*U ) / CP
   20   CONTINUE
        FMT         = '(F7.1)  '
        NPROP       = 'ABSOLUTE STAGNATION TEMPERATURE '
      ELSE IF(IREPLY . EQ . 21) THEN
        ...
        DO 21 I=1,IM
          ...
          T0REL       = P(I,J,K)/(RO(I,J,K)*GC) * FMACH
          U           = W(J)*R(J,K)
          T0          = T0REL + ( U*VT(I,J,K) + 0.5*U*U ) / CP
          PROP(I,J,K) = P(I,J,K)*(FMACH*T0/T0REL)**(GA/(GA-1.0))
   21   CONTINUE
        FMT         = '(F9.1)  '
        NPROP       = '  ABSOLUTE STAGNATION PRESSURE  '
...
C
      ELSE IF(IREPLY . EQ . 27) THEN
        ...
        DO 27 I=1,IM
          U           = W(J)*R(J,K)
C
C       Calculate Absolute Pitchwise Flow angle
C
          VMER = SQRT ( VX(I,J,K)**2 + VR(I,J,K)**2 )
          VTAB = VT(I,J,K) + U
C
C          IF (VMER . EQ . 0.0) THEN
C              IF (VTAB.LT.0.0) THEN
C                  BETA = -1.57080
C              ELSE IF (VTAB . EQ . 0.0) THEN
C                  JP1    = J+1
C                  JM1    = J-1
C                  IF (JM1.LT.1) JM1    =  1
C                  IF (JP1.GT.JM) JP1   = JM
C                  BETA  = ATAN2((R(J,K)*T(I,JP1,K)
C     &                          -R(J,K)*T(I,JM1,K)),
C     &                          (SM(JP1,K)-SM(JM1,K)))
C              ELSE IF (VTAB.GT.0.0) THEN
C                  BETA  = 1.57080
C              ENDIF
C
C          ELSE
...
C
          PROP(I,J,K) = BETA * DEG
C        IF(K.EQ.19.AND.J.EQ.140.AND.I.EQ.19) WRITE(6,*) VT(I,J,K),U,BETA
   27   CONTINUE
        FMT         = '(F7.1)  '
...
C
        WRITE (6,2083) IMID, KMID
C
        FMACH  = (1.0+(GA-1.0)*0.5*MACH(imid,1,kmid)**2)
c
        uin   = W(1)*r(1, kmid)
        toin  = p(imid,1,kmid)/(ro(imid,1,kmid)*gc)*FMACH        
        poin  = p(imid,1,kmid)*FMACH**(ga/(ga-1.))
        torin = toin - 0.5*uin**2/cp
c
        DO 28 K=1,KM
        DO 28 J=1,JM
        DO 28 I=1,IM
             U    = W(J)*R(J,K)
             topt = torin + 0.5*u**2/cp
             popt = (topt/toin)**(ga/(ga-1.0))*poin    
             IF (popt/p(i, j, k).LT.1.0) THEN
                 PROP(I,J,K) = 0.0
             ELSE
                 PROP(I,J,K) = SQRT(((popt/p(i, j, k))**
     &                         ((ga-1.)/ga)-1.)*2./(ga-1.))
             ENDIF
   28   CONTINUE
c
...
C
C ENTROPY LOSS COEFFICIENT CALCULATION
C
...
C        
        ...
        DO 555 J=1,JM
        DO 555 I=1,IM
          TSTAT  = P(I,J,K)/GC/RO(I,J,K)/288.0
          PSTAT  = P(I,J,K)/100000.
          ENTPY  = CP*DLOG(TSTAT) - GC*DLOG(PSTAT)
          PROP2(I,J,K) = ENTPY
          FMACH = MACH(I,J,K)*MACH(I,J,K)*GC*GA
          PROP3(I,J,K) = 0.5*FMACH
  555   CONTINUE
C
         WRITE(6,*) ' INPUT THE REFERENCE J VALUE FOR THE ENTROPY'
         READ(5,*)    JLOS1
         CALL SUB23(PROP2,0)
         CALL SUB23(PROP3,0)
         DO 556 J=1,JM
         DO 556 K=1,KM
         DO 556 I=1,IM
         PROP(I,J,K) = (PROP2(I,J,K) - PROP2(I,JLOS1,K))/PROP3(I,J,K)
  556    CONTINUE
C
...
C
      ...
      DO 220 K=1,KM
      DO 220 I=1,IM
          VTABS= VT(I,J,K) + W(J)*R(J,K)      
          VABSQ= VX(I,J,K)*VX(I,J,K) +VR(I,J,K)*VR(I,J,K)+VTABS*VTABS
          ABMSQ = VABSQ*RO(I,J,K)/(GA*P(I,J,K))
          FMACH  = (1.0+(GA-1.0)*0.5*ABMSQ)
          PROP(I,J,K) = P(I,J,K)*FMACH**(GA/(GA-1.0))
          PROP2(I,J,K) = P(I,J,K)/(RO(I,J,K)*GC) +0.5*VABSQ/CP
  220 CONTINUE
C
C      PROP IS NOW THE STAGNATION PRESSURE
C      PROP2  IS THE STAGNATION TEMPERATURE
C
C     FIND THE INLET MASS AVERAGE  PO AND TO
C
      J=1
C
      DO 225 K=1,KM
...
C
      ...
      IF(IREPLY.EQ.30) GO TO 333
C
C      MASS AVERAGE THE STAGNATION PRESSURE AND TEMPERATURE
C
      CALL SUB23(PROP,0)
      CALL SUB23(PROP2,0)
C
...
C
      ...
      DO 226 K=1,KM
      DO 226 I=1,IM
      RATPO=PROP(I,J,K)/POINPUT(K)
      IF(RATPO.LT.0.0) RATPO = 0.005
      IF(ABS(RATPO-1.0).LT.0.005) GO TO 227
      RATTO=PROP2(I,J,K)/TOINPUT(K)
      IF(RATTO.LT.0.0) RATTO = 0.005
      IF(ABS(RATTO-1.0).LT.0.005) GO TO 227
      POWER= ALOG(RATTO)/ALOG(RATPO)
      POLY=POWER*GA/(GA-1.)
...
C
      ENDIF
C
  666 FORMAT( ' DO YOU WANT TO MASS AVERAGE THE VARIABLE ? ',
     & /,' Answer Y  OR  N  OR  D',
     & /,' Y means pitchwise average D means passage average ',
     & /,' Default =  N ')
  667 FORMAT(1A)
C
      WRITE(13,*)
      WRITE(13,*)  ' VALUES OF THE VARIABLE ',  NPROP
      WRITE(13,*)  ' RADIUS VALUES, VARIABLE VALUES '
      DO K=1,KM
      WRITE(13,2084)  R(JM,K), PROP(1,JM-1,K)
      END DO 
      WRITE(13,*)
 2084 FORMAT(2F15.5)
C
...
C
     ...
     &                JTEE(50),NROWS
C
      DIMENSION PROP(95,2500,95),AVG(95),FAVG(95)
C
C
      IMM1= IM-1
      KMM1 =KM-1
...
C
          DO 1311 K = 1,KMM1
             DO 1312 J = 1,JMM1
                 READ  (4,4003,ERR=999,END=999) (MACH(I,J,K),I = 1,IMM1)
 1312        CONTINUE
 1311     CONTINUE
C
C          Static pressure
...
C
C
      DIMENSION VAR(95,2500,95), TEMP(95,2500,95)
C
C
      COMMON / BLK15/ IM,JM,KM,ISPLIT,KSPLIT,JLE,JTE,JSLE,JSTE,CP,GA,
     &                W(2500),IND(2500),NBLADES(2500),PI,JLEE(50),
...
C
     ...
     &                JTEE(50),NROWS
C
      DIMENSION SOSM1(2500),SOSM2(2500)
C
      CHARACTER*32 NSOSM
C
...
C
      ...
      CHARACTER*1  CANS
C
      DIMENSION PROP(95,2500,95),HITES(125),STOREX(95,2500),
     &          STORER(95,2500)
C
      REAL EMACH,IMACH
C
...
C
     ...
     &            F12(95),  F13(95),
     &            PIF0(95,2500), PIF1(95,2500), PIF2(95,2500),  
     &            PIF3(95,2500), PIF4(95,2500), PIF5(95,2500), 
     &            PIF6(95,2500), PIF7(95,2500), 
     &            PIF9(95,2500), PIF10(95,2500),PIF11(95,2500),
     &            PIF12(95,2500), PIF13(95,2500),
     &            SPIF0R(2500) ,SPIF1R(2500), SPIF2R(2500), 
     &            SPIF3R(2500), SPIF4R(2500), SPIF5R(2500),
     &            SPIF6R(2500), SPIF7R(2500),
     &            SPIF9R(2500),SPIF10R(2500),SPIF11R(2500),
     &            SPIF12R(2500),SPIF13R(2500),
     &            SPIF0X(2500) ,SPIF1X(2500), SPIF2X(2500), 
     &            SPIF3X(2500), SPIF4X(2500), SPIF5X(2500),
     &            SPIF6X(2500), SPIF7X(2500), 
     &            SPIF9X(2500),SPIF10X(2500),SPIF11X(2500),
     &            SPIF12X(2500),SPIF13X(2500),
     &            dx(95,2500), dr(95,2500),
     &            xvar(95), yvar(95)
C
C
       cv = cp / ga
